...
sig
  ...
  val listItemsi: 'a table -> (symbol * 'a) list (* PRINT *)
...
fun listItemsi t = (* PRINT *)
    let
        val keyvals = H.listItemsi hashtable
        fun int2sym [] n = raise Symbol
          | int2sym ((s, n)::rest) n' =
            if n = n' then (s, n) else int2sym rest n'
    in
        List.map (fn (n, v) => (int2sym keyvals n, v)) (Table.listItemsi t)
    end
