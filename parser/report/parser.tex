...
%In this test the let declaration is left unclosed. The parser responds to this by closing the let statement by inserting IN and END, as per the %change directive.
...
% This test demonstrates an example where the parser uses a preferred substitution of EQ with ASSIGN.

\subsection{test3 - Forgot using then in a if}
\begin{lstlisting}[frame=single]if a<>0 1 else 2
\end{lstlisting}

% This test demonstrates an example where the parser inserts the preferred THEN keyword.
